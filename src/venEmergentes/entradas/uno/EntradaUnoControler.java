package venEmergentes.entradas.uno;
...
public class EntradaUnoControler extends Funciones  implements Initializable {
	
	public static EntradaUnoControler controler=null;
	public  EntradaUnoControler cargarModal(int opcion, String  titulo, String subtitulo){
		FXMLLoader fXMLLoader=new FXMLLoader();
		fXMLLoader.setLocation(getClass().getResource("/venEmergentes/entradas/uno/EntradaUno.fxml"));
		try {
			fXMLLoader.load();
			EntradaUnoControler.controler=fXMLLoader.getController();
			EntradaUnoControler.controler.setOpcionSeleccionada(opcion);
			EntradaUnoControler.controler.labelTitulo.setText(titulo);
			EntradaUnoControler.controler.labelEntrada1.setText(subtitulo);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static final int TIENE_CONOCIMIENTO_EN=10;
	public static final int NECESITA_APRENDER=11;
...
public String insertarDatos(){
	  ...
	  try{     
		  ...
		  if(opcionSeleccionada==EntradaUnoControler.PROGRAMA_SOCIAL){
           pst = conn.prepareStatement("INSERT INTO PROGRAMA_SOCIAL(NOMBRE) VALUES (?);"); 
		  }else if(opcionSeleccionada==EntradaUnoControler.PROCEDENCIA){
	           pst = conn.prepareStatement("INSERT INTO TIPO_PROCEDENCIA(NOMBRE) VALUES (?);"); 
		  }else if(opcionSeleccionada==EntradaUnoControler.TIPO_VIA){
			  pst = conn.prepareStatement("INSERT INTO TIPO_VIA(NOMBRE) VALUES (?);");
		  }else if(opcionSeleccionada==EntradaUnoControler.TIPO_DISCAPACIDAD){
			  pst = conn.prepareStatement("INSERT INTO TIPO_DISCAPACIDAD(NOMBRE) VALUES (?);");
		  }else if(opcionSeleccionada==EntradaUnoControler.LIMITACION_PERMANENTE){
			  pst = conn.prepareStatement("INSERT INTO LIMITACION_PERMANENTE_PARA(NOMBRE) VALUES (?);");
		  }else if(opcionSeleccionada==EntradaUnoControler.REQUERIMIENTO_APOYO){
			  pst = conn.prepareStatement("INSERT INTO REQUERIMIENTO_APOYO(NOMBRE)VALUES(?);");
		  }else if(opcionSeleccionada==EntradaUnoControler.TIPO_AYUDA_BIOMETRICA){
			  pst = conn.prepareStatement("INSERT INTO TIPO_AYUDA_BIOMECANICA(NOMBRE) VALUES (?);");
		  }else if(opcionSeleccionada==EntradaUnoControler.TIPO_SEGURO_MEDICO){
			  pst = conn.prepareStatement("INSERT INTO TIPO_SEGURO(NOMBRE) VALUES (?);");
		  }else if(opcionSeleccionada==EntradaUnoControler.ESTABLECIMIENTO_SALUD){
			  pst = conn.prepareStatement("INSERT INTO establecimiento_salud( NOMBRE) VALUES (?)");
		  }else if(opcionSeleccionada==EntradaUnoControler.TIENE_CONOCIMIENTO_EN){
			  pst = conn.prepareStatement("INSERT INTO TIENE_CONOCIMIENTO_EN( NOMBRE) VALUES (?)");
		  }else if(opcionSeleccionada==EntradaUnoControler.NECESITA_APRENDER){
			  pst = conn.prepareStatement("INSERT INTO NECESITA_APRENDER( NOMBRE) VALUES (?)");
		  }
