package estadistica.nivel_educativo;
...
public class EstadisticaNivelEducativoControler extends Funciones implements Initializable {
	...
	public void cargarGraficaNivelEducativo(){
		...
		barChartNivelEducativo.setTitle("Número (n) de Personas - Nivel Educativo");
		numberAxisNivelEducativo.setLabel("Número de Personas(n)");
		categoryAxisNivelEducativo.setLabel("Nivel Educativo");
		...
		try {
			...
			while (rs.next()) {
				...
				if(rs.getInt("SEXO")==1/*Masculino*/){
					...
					seriesNivelEducativo_Masculino.getData().add(new XYChart.Data<String, Number>(rs.getString("NIVEL_EDUCATIVO"), rs.getInt("NUMERO")));

				}else if(rs.getInt("SEXO")==2)/*Femenino*/{
					seriesNivelEducatio_Femenino.getData().add(new XYChart.Data<String, Number>(rs.getString("NIVEL_EDUCATIVO"), rs.getInt("NUMERO")));
				}
			}
